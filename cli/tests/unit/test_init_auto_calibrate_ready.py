...
def _write_manifest(
    tmp_path: Path, *, include_ready: bool = False, ik_provider: str | None = "inhouse-so-arm101"
) -> Path:
    fm = {
...
def _patch_other_phases(monkeypatch, init_module):
    ...
    monkeypatch.setattr(
        init_module, "phase_write_manifest", ok_factory("write_manifest"), raising=False
    )
    for name in ("phase_calibrate_sign", "phase_calibrate_zero", "phase_teach_poses"):
        monkeypatch.setattr(
            init_module, name, skip_factory(name.replace("phase_", "")), raising=False
        )
