...
def test_full_sweep_produces_calibrated_manifest(tmp_path, monkeypatch):
    ...
    K = np.array([[200.0, 0.0, 320.0], [0.0, 200.0, 400.0], [0.0, 0.0, 1.0]])
    ...
    def _grab_frame():
        ...
        rad_cfg: dict[str, float] = {
            j.id: j.steps_to_rad(current.get(j.id, j.zero_pose_steps)) for j in kin.joints
        }
        ...
        u = round(K[0, 0] * tip_cam[0] / tip_cam[2] + K[0, 2])
        v = round(K[1, 1] * tip_cam[1] / tip_cam[2] + K[1, 2])

        if 0 <= u < WIDTH and 0 <= v < HEIGHT:
            yy, xx = np.ogrid[:HEIGHT, :WIDTH]
            # Radius 8px gives >50 pixels — well above the 5-point threshold.
            mask = np.hypot(xx - u, yy - v) <= 8
            depth[mask] = round(tip_cam[2])
    ...
    assert "extrinsic_residual_mm" in post_cam0, "extrinsic_residual_mm not written to manifest"
